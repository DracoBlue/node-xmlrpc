...
xmlrpcParser.parseMethodResponse = function(xml, callback) {
	...
	var saxParser = new xmlParser.SaxParser(function(cb) {
		...
		cb.onEndDocument(function() {
			if (stack[0]["values"][0]["values"][0].name === 'fault') {
				var fault_object = stack[0]["values"][0]["values"][0];
				callback(fault_object["values"][0]);
			} else {
		...
		cb.onEndElementNS(function(elem, prefix, uri) {
			...
			if (last_element.name === 'fault') {
				/*
				 * Because it's always fault>value
				 */
				var struct_values = [];
				...
				last_element = {"name": "fault", "values": struct_values};
			}
